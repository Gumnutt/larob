...
const librariesInput = Object.keys(librariesYaml)
  ...
  .map((v) => [...Object.keys(librariesYaml[v].css?.theme), ...Object.keys(librariesYaml[v].js)])
  .flat()
    ...
    async handleHotUpdate({ file, server }) {
      ...
      if (file.includes("components") && file.endsWith(".pcss")) {
        ...
        try {
          ...
          const result = await postcss([postcssImport(), postcssNesting(), tailwindcss(), autoprefixer()]).process(css, { from: undefined })
...
export default ({ mode }) => {
  ...
  const config = {
    ...
    css: {
      ...
      preprocessorOptions: {
        pcss: {
          additionalData: `@import '/assets/pcss/base/variables.css';`,
        },
      },
      postcss: {
